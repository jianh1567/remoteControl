package com.wind.control.widget.timeselector;
...
import com.wind.control.widget.timeselector.Utils.DateUtil;
import com.wind.control.widget.timeselector.Utils.ScreenUtil;
import com.wind.control.widget.timeselector.Utils.TextUtil;
import com.wind.control.widget.timeselector.view.PickerView;
...
public class TimeSelector {
    ...
    public TimeSelector(Context context, ResultHandler resultHandler, String select_year, String select_mouth, String select_day) {
        this.context = context;
        this.handler = resultHandler;
        this.select_year = select_year;
        this.select_mouth = select_mouth;
        this.select_day = select_day;
        startCalendar = Calendar.getInstance();
        endCalendar = Calendar.getInstance();
        startCalendar.setTime(DateUtil.parse("1900-01-01 00:00", FORMAT_STR));
        endCalendar.setTime(DateUtil.parse("2030-12-31 00:00", FORMAT_STR));
        initDialog();
        initView();
    }
...
//        });
        month_pv.setOnSelectListener(new PickerView.onSelectListener() {
            ...
            public void onSelect(String text) {
                ...
                select_mouth = text;
                if (mLastDay == 0) {
                    mLastDay = Integer.parseInt(select_day);
                }
                int year = Integer.parseInt(select_year);
                try {
                    int days = getDaysOfMonth(sdf.parse(year + "-" + text));
                    if (mLastDay > days) {
                        mLastDay = 1;
                    }
                    selectedCalender.set(Calendar.DAY_OF_MONTH, mLastDay);
                    dayChange();
    ...
    private void loadComponent() {
        ...
        if (TextUtils.isEmpty(select_year)) {
            year_pv.setSelected(79);
            selectedCalender.set(Calendar.YEAR, 1979);
        } else {
            int sel_year = Integer.parseInt(select_year) - 1900;
            year_pv.setSelected(sel_year);
            selectedCalender.set(Calendar.YEAR, Integer.parseInt(select_year));
        }

        if (TextUtils.isEmpty(select_mouth)) {
            month_pv.setSelected(0);
            selectedCalender.set(Calendar.DAY_OF_MONTH, 1);
            selectedCalender.set(Calendar.MONTH, 0);
            dayChange();
        } else {
            if (select_mouth.substring(0, 1).equals("0")) {
                int sel_mouth = Integer.parseInt(select_mouth.substring(1, 2)) - 1;
                month_pv.setSelected(sel_mouth);
                selectedCalender.set(Calendar.DAY_OF_MONTH, 1);
                selectedCalender.set(Calendar.MONTH, sel_mouth);
            } else {
                int sel_mouth = Integer.parseInt(select_mouth) - 1;
                month_pv.setSelected(sel_mouth);
                selectedCalender.set(Calendar.DAY_OF_MONTH, 1);
                selectedCalender.set(Calendar.MONTH, sel_mouth);
            }
        ...
        if (TextUtils.isEmpty(select_day)) {
            //  day_pv.setSelected(0);
            //  selectedCalender.set(Calendar.DAY_OF_MONTH, 01);
        } else {
            if (select_day.substring(0, 1).equals("0")) {
                int sel_day = Integer.parseInt(select_day.substring(1, 2)) - 1;
                day_pv.setSelected(sel_day);
                selectedCalender.set(Calendar.DAY_OF_MONTH, Integer.parseInt(select_day.substring(1, 2)));
            } else {
                int sel_day = Integer.parseInt(select_day) - 1;
                day_pv.setSelected(sel_day);
                selectedCalender.set(Calendar.DAY_OF_MONTH, Integer.parseInt(select_day));
            }
        }
